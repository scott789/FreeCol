...
public class Player extends FreeColGameObject implements Nameable {
    ...
    protected PlayerData data = new PlayerData(null, false, false, 0, 0, new HashSet<>(), new ArrayList<>(), new HashMap<>(),
			null, new HashMap<>(), new ArrayList<>(), new ArrayList<>(), new ArrayList<>(), null, new ArrayList<>(), new ArrayList<>(), null,
			false, new Object(), new FeatureContainer(), -1, new UnitIterator(this, new ActivePredicate()), new UnitIterator(this, new GoingToPredicate(this, false)), new UnitIterator(this, new GoingToPredicate(this, true)), null,
			new HashMap<>());
    ...
    public boolean isUnknownEnemy() {
        return Nation.UNKNOWN_NATION_ID.equals(data.nationId);
    }
    ...
    public final String getIndependentNationName() {
        return data.independentNationName;
    }
    ...
    public final void setIndependentNationName(final String newIndependentNationName) {
        this.data.independentNationName = newIndependentNationName;
    }
    ...
    public void setNewLandName(String newLandName) {
        this.data.newLandName = newLandName;
    }
    ...
    public String getEuropeNameKey() {
        return (data.europe == null) ? null : data.nationId + ".europe";
    }
    ...
    public String getNationResourceKey() {
        return lastPart(data.nationId, ".").toUpperCase(Locale.US);
    }
    ...
    public StringTemplate getNationLabel() {
        return (data.playerType == PlayerType.REBEL
                || data.playerType == PlayerType.INDEPENDENT)
            ? StringTemplate.name(data.independentNationName)
            : StringTemplate.key(Messages.nameKey(data.nationId));
    }
    ...
    public final String getRulerNameKey() {
        return Messages.rulerKey(data.nationId);
    }
    ...
    public String getMonarchKey() {
        return "image.flavor.monarch." + data.nationId;
    }
    ...
    public void changePlayerType(PlayerType type) {
        if (data.playerType != PlayerType.REBEL
            && data.playerType != PlayerType.INDEPENDENT) {
            switch (type) {
    ...
    public boolean isColonial() {
        return data.playerType == PlayerType.COLONIAL;
    }
    ...
    public boolean isEuropean() {
        return data.playerType == PlayerType.COLONIAL
            || data.playerType == PlayerType.REBEL
            || data.playerType == PlayerType.INDEPENDENT
            || data.playerType == PlayerType.ROYAL;
    }
    ...
    public boolean isIndian() {
        return data.playerType == PlayerType.NATIVE;
    }
    ...
    public boolean isUndead() {
        return data.playerType == PlayerType.UNDEAD;
    }
    ...
    public boolean isREF() {
        return data.playerType == PlayerType.ROYAL;
    }
    ...
    public void changeNationType(NationType newNationType) {
        if (data.nationType != null) removeFeatures(data.nationType);
    ...
    public boolean canBuildColonies() {
        return data.nationType.hasAbility(Ability.FOUNDS_COLONIES);
    }
    ...
    public boolean canHaveFoundingFathers() {
        return data.nationType.hasAbility(Ability.ELECT_FOUNDING_FATHER);
    }
    ...
    public Nation getNation() {
        return getSpecification().getNation(data.nationId);
    }
    ...
    public boolean getAttackedByPrivateers() {
        return data.attackedByPrivateers;
    }
    ...
    public void setAttackedByPrivateers(boolean attacked) {
        data.attackedByPrivateers = attacked;
    }
    ...
    public boolean checkGold(int amount) {
        return this.data.gold == GOLD_NOT_ACCOUNTED || this.data.gold >= amount;
    }
    ...
    public int modifyGold(int amount) {
        if (this.data.gold != Player.GOLD_NOT_ACCOUNTED) {
    ...
    public final void setBankrupt(final boolean newBankrupt) {
        this.data.bankrupt = newBankrupt;
    }
    ...
    public int getImmigration() {
        return (isColonial()) ? data.immigration : 0;
    }
    ...
    public void setImmigration(int immigration) {
        ...
        this.data.immigration = immigration;
    }
    ...
    public void reduceImmigration() {
        ...
        int cost = getSpecification()
            ...
            ? data.immigrationRequired : data.immigration;
        if (cost > data.immigration) {
            data.immigration = 0;
        } else {
            data.immigration -= cost;
        }
    }
    ...
    public void modifyImmigration(int amount) {
        data.immigration = Math.max(0, data.immigration + amount);
    }
    ...
    public int getImmigrationRequired() {
        return data.immigrationRequired;
    }
    ...
    public void setImmigrationRequired(int immigrationRequired) {
        this.data.immigrationRequired = immigrationRequired;
    }
    ...
    public void updateImmigrationRequired() {
        ...
        final int current = data.immigrationRequired;
        ...
        data.immigrationRequired = (int)applyModifiers(unreduced + base, turn,
            Modifier.RELIGIOUS_UNREST_BONUS);;
        logger.finest("Immigration for " + getId() + " updated " + current
            + " -> " + data.immigrationRequired);
    }
    ...
    public boolean checkEmigrate() {
        return (isColonial()) ? getImmigrationRequired() <= data.immigration
            : false;
    }
    ...
    public int getLiberty() {
        return (canHaveFoundingFathers()) ? data.liberty : 0;
    }
    ...
    public void modifyLiberty(int amount) {
        setLiberty(Math.max(0, data.liberty + amount));
        if (isRebel()) data.interventionBells += amount;
    }
    ...
    public boolean hasFather(FoundingFather someFather) {
        return data.foundingFathers.contains(someFather);
    }
    ...
    public int getFatherCount() {
        return data.foundingFathers.size();
    }
    ...
    public void addFather(FoundingFather father) {
        data.foundingFathers.add(father);
        addFeatures(father);
    ...
    public void setOfferedFathers(List<FoundingFather> fathers) {
        clearOfferedFathers();
        data.offeredFathers.addAll(fathers);
    }
    ...
    public void reinitialiseMarket() {
        data.market = new Market(getGame(), this);
    }
    ...
    public LastSale getLastSale(Location where, GoodsType what) {
        return (data.lastSales == null) ? null
            : data.lastSales.get(LastSale.makeKey(where, what));
    }
    ...
    public void addLastSale(LastSale sale) {
        if (data.lastSales == null) data.lastSales = new HashMap<>();
        data.lastSales.put(sale.getId(), sale);
    }
            ...
            for (Goods g : colony.getCompactGoods()) {
                if (getArrears(g.getType()) <= 0 && hasTraded(g.getType())) {
                    int amount = Math.min(g.getAmount(),
                                          GoodsContainer.CARGO_SIZE);
                    int value = data.market.getSalePrice(g.getType(), amount);
                    if (value > highValue) {
                        highValue = value;
                        goods = g;
                    }
    ...
    public boolean hasUnit(Unit unit) {
        synchronized (data.units) {
            return data.units.contains(unit);
        }
    }
    ...
    public List<Unit> getUnits() {
        synchronized (data.units) {
            return new ArrayList<>(data.units);
        }
    }
    ...
    public Unit getUnitByName(String name) {
        return find(data.units, u -> name.equals(u.getName()));
    }
    ...
    public boolean removeUnit(final Unit oldUnit) {
        ...
        data.nextActiveUnitIterator.remove(oldUnit);
        data.nextGoingToUnitIterator.remove(oldUnit);
        data.nextTradeRouteUnitIterator.remove(oldUnit);

        synchronized (data.units) {
            return data.units.remove(oldUnit);
        }
    }
    ...
    public Unit getNextActiveUnit() {
        return data.nextActiveUnitIterator.next();
    }
    ...
    public boolean setNextActiveUnit(Unit unit) {
        return data.nextActiveUnitIterator.setNext(unit);
    }
    ...
    public boolean hasNextActiveUnit() {
        return data.nextActiveUnitIterator.hasNext();
    }
    ...
    public Unit getNextGoingToUnit() {
        return data.nextGoingToUnitIterator.next();
    }
    ...
    public boolean setNextGoingToUnit(Unit unit) {
        return data.nextGoingToUnitIterator.setNext(unit);
    }
    ...
    public boolean hasNextGoingToUnit() {
        return data.nextGoingToUnitIterator.hasNext();
    }
    ...
    public Unit getNextTradeRouteUnit() {
        return data.nextTradeRouteUnitIterator.next();
    }
    ...
    public boolean hasNextTradeRouteUnit() {
        return data.nextTradeRouteUnitIterator.hasNext();
    }
    ...
    public void resetIterators() {
        data.nextActiveUnitIterator.reset();
        data.nextGoingToUnitIterator.reset();
        data.nextTradeRouteUnitIterator.reset();
    }
    ...
    public TradeRoute getTradeRouteByName(String name) {
        return find(data.tradeRoutes, t -> t.getName().equals(name));
    }
    ...
    public final void setTradeRoutes(final List<TradeRoute> newTradeRoutes) {
        data.tradeRoutes.clear();
        data.tradeRoutes.addAll(newTradeRoutes);
    }
    ...
    public boolean hasSettlement(Settlement settlement) {
        return data.settlements.contains(settlement);
    }
    ...
    public boolean removeSettlement(Settlement settlement) {
        return data.settlements.remove(settlement);
    }
    ...
    public List<ModelMessage> getModelMessages() {
        synchronized (data.modelMessages) {
            return new ArrayList<>(data.modelMessages);
        }
    }
    ...
    public List<ModelMessage> getNewModelMessages() {
        synchronized (data.modelMessages) {
            return data.modelMessages.stream()
    ...
    public void addModelMessage(ModelMessage modelMessage) {
        synchronized (data.modelMessages) {
            data.modelMessages.add(modelMessage);
        }
    }
    ...
    public void refilterModelMessages(OptionGroup options) {
        synchronized (data.modelMessages) {
            Iterator<ModelMessage> messageIterator = data.modelMessages.iterator();
    ...
    public void setEntryLocation(Location entryLocation) {
        this.data.entryLocation = entryLocation;
    }
    ...
    public void initializeHighSeas() {
        Game game = getGame();
        data.highSeas = new HighSeas(game);
        if (data.europe != null) data.highSeas.addDestination(data.europe);
        if (game.getMap() != null ) data.highSeas.addDestination(game.getMap());
    }
    ...
    public boolean canSee(Tile tile) {
        ...
        do {
            synchronized (data.canSeeLock) {
                if (data.canSeeValid) {
                    return data.canSeeTiles[tile.getX()][tile.getY()];
                }
            }
    ...
    private boolean resetCanSeeTiles() {
        ...
        synchronized (data.canSeeLock) {
            data.canSeeTiles = cST;
            data.canSeeValid = true;
        }
        return true;
    }
    ...
    private boolean[][] makeCanSeeTiles(Map map) {
        ...
        if (!spec.getBoolean(GameOptions.FOG_OF_WAR)) {
            boolean[][] cST = (data.canSeeTiles != null) ? data.canSeeTiles
    ...
    public Tension getTension(Player player) {
        ...
        Tension newTension = data.tension.get(player);
        if (newTension == null) {
            newTension = new Tension(Tension.TENSION_MIN);
            data.tension.put(player, newTension);
        }
        return newTension;
    }
    ...
    public void removeTension(Player player) {
        if (player != null) data.tension.remove(player);
    }
    ...
    public boolean missionsBanned(Player player) {
        return data.bannedMissions != null && data.bannedMissions.contains(player);
    }
    ...
    public void addMissionBan(Player player) {
        if (data.bannedMissions == null) data.bannedMissions = new HashSet<>();
        data.bannedMissions.add(player);
    }
    ...
    public void removeMissionBan(Player player) {
        if (data.bannedMissions != null) data.bannedMissions.remove(player);
    }
    ...
    public Stance getStance(Player player) {
        return (player == null || data.stance.get(player.getId()) == null)
            ? Stance.UNCONTACTED
            : data.stance.get(player.getId());
    }
    ...
    public boolean setStance(Player player, Stance newStance) {
        ...
        if (newStance == null) {
            data.stance.remove(player.getId());
            return true;
        }
        Stance oldStance = data.stance.get(player.getId());
        ...
        data.stance.put(player.getId(), newStance);
        return valid;
    }
    ...
    public static void makeContact(Player player1, Player player2) {
        player1.data.stance.put(player2.getId(), Stance.PEACE);
        player2.data.stance.put(player1.getId(), Stance.PEACE);
    ...
    public NationSummary getNationSummary(Player player) {
        return data.nationCache.get(player);
    }
    ...
    public void putNationSummary(Player player, NationSummary ns) {
        data.nationCache.put(player, ns);
    }
    ...
    */
    ...
    public List<Double> getAllColonyValues(Tile tile) {
        ...
        double development = Math.min(LOW_SETTLEMENT_NUMBER, data.settlements.size())
                / (double)LOW_SETTLEMENT_NUMBER;
        int portCount = getNumberOfPorts();

        if (tile.isPolar() && data.settlements.size() < LOW_SETTLEMENT_NUMBER) {
    ...
    public int getMaximumFoodConsumption() {
        if (data.maximumFoodConsumption < 0) {
            ...
            for (UnitType unitType : spec.getUnitTypeList()) {
                if (unitType.isAvailableTo(this)) {
                    ...
                    if (foodConsumption > data.maximumFoodConsumption) {
                        data.maximumFoodConsumption = foodConsumption;
                    }
                }
            }
        }
        return data.maximumFoodConsumption;
    }
    ...
    public int checkIntegrity(boolean fix) {
        ...
        if (data.monarch != null) {
            result = Math.min(result, data.monarch.checkIntegrity(fix));
        }
        return result;
    }
    ...
    protected void writeAttributes(FreeColXMLWriter xw) throws XMLStreamException {
        ...
        xw.writeAttribute(USERNAME_TAG, data.name);

        xw.writeAttribute(NATION_ID_TAG, data.nationId);

        if (data.nationType != null) {
            xw.writeAttribute(NATION_TYPE_TAG, data.nationType);
        }

        xw.writeAttribute(ADMIN_TAG, data.admin);

        xw.writeAttribute(READY_TAG, data.ready);

        xw.writeAttribute(DEAD_TAG, data.dead);

        xw.writeAttribute(PLAYER_TYPE_TAG, data.playerType);

        xw.writeAttribute(AI_TAG, data.ai);

        if (xw.validFor(this)) {

            xw.writeAttribute(BANKRUPT_TAG, data.bankrupt);

            xw.writeAttribute(TAX_TAG, data.tax);

            xw.writeAttribute(GOLD_TAG, data.gold);

            xw.writeAttribute(IMMIGRATION_TAG, data.immigration);

            xw.writeAttribute(LIBERTY_TAG, data.liberty);

            xw.writeAttribute(INTERVENTION_BELLS_TAG, data.interventionBells);

            if (data.currentFather != null) {
                xw.writeAttribute(CURRENT_FATHER_TAG, data.currentFather);
            }

            xw.writeAttribute(IMMIGRATION_REQUIRED_TAG, data.immigrationRequired);

            xw.writeAttribute(ATTACKED_BY_PRIVATEERS_TAG, data.attackedByPrivateers);

            xw.writeAttribute(OLD_SOL_TAG, data.oldSoL);

            xw.writeAttribute(SCORE_TAG, data.score);
        }

        if (data.newLandName != null) {
            xw.writeAttribute(NEW_LAND_NAME_TAG, data.newLandName);
        }

        if (data.independentNationName != null) {
            xw.writeAttribute(INDEPENDENT_NATION_NAME_TAG, data.independentNationName);
        }

        if (data.entryLocation != null) {
            xw.writeLocationAttribute(ENTRY_LOCATION_TAG, data.entryLocation);
        }
    }
    ...
    protected void writeChildren(FreeColXMLWriter xw) throws XMLStreamException {
        ...
        if (data.market != null) data.market.toXML(xw);
        ...
        if (xw.validFor(this)) {
            ...
            for (Player p : getSortedCopy(data.tension.keySet())) {
                xw.writeStartElement(TENSION_TAG);

                xw.writeAttribute(PLAYER_TAG, p);

                xw.writeAttribute(VALUE_TAG, data.tension.get(p).getValue());

                xw.writeEndElement();
            }
            
            if (data.bannedMissions != null) {
                for (Player p : getSortedCopy(data.bannedMissions)) {
            ...
            for (Entry<String, Stance> e : mapEntriesByKey(data.stance)) {
                Stance s = e.getValue();
            ...
            for (HistoryEvent event : data.history) { // Already in order
                event.toXML(xw);
            }

            for (TradeRoute route : getSortedCopy(data.tradeRoutes)) {
                route.toXML(xw);
            }

            if (data.highSeas != null) data.highSeas.toXML(xw);
            
            xw.writeToListElement(FOUNDING_FATHERS_TAG, data.foundingFathers);

            xw.writeToListElement(OFFERED_FATHERS_TAG, data.offeredFathers);

            if (data.europe != null) data.europe.toXML(xw);

            if (data.monarch != null) data.monarch.toXML(xw);

            for (ModelMessage m : getModelMessages()) m.toXML(xw);

            if (data.lastSales != null) {
                for (LastSale ls : getSortedCopy(data.lastSales.values())) {
                    ls.toXML(xw);
                }
            }
    ...
    protected void readAttributes(FreeColXMLReader xr) throws XMLStreamException {
        ...
        data.name = xr.getAttribute(USERNAME_TAG, (String)null);

        data.nationId = xr.getAttribute(NATION_ID_TAG,
            // @compat 0.10.7
        ...
        if (isUnknownEnemy()) {
            data.nationType = null;
        } else {
            data.nationType = xr.getType(spec, NATION_TYPE_TAG,
                                    NationType.class, (NationType)null);
        }

        data.admin = xr.getAttribute(ADMIN_TAG, false);

        data.gold = xr.getAttribute(GOLD_TAG, 0);

        data.immigration = xr.getAttribute(IMMIGRATION_TAG, 0);

        data.liberty = xr.getAttribute(LIBERTY_TAG, 0);

        data.interventionBells = xr.getAttribute(INTERVENTION_BELLS_TAG, 0);

        data.oldSoL = xr.getAttribute(OLD_SOL_TAG, 0);

        data.score = xr.getAttribute(SCORE_TAG, 0);

        data.ready = xr.getAttribute(READY_TAG, false);

        data.ai = xr.getAttribute(AI_TAG, false);

        data.dead = xr.getAttribute(DEAD_TAG, false);

        data.bankrupt = xr.getAttribute(BANKRUPT_TAG, false);

        data.tax = xr.getAttribute(TAX_TAG, 0);

        changePlayerType(xr.getAttribute(PLAYER_TYPE_TAG,
                                         PlayerType.class, (PlayerType)null));

        data.currentFather = xr.getType(spec, CURRENT_FATHER_TAG,
                                   FoundingFather.class, (FoundingFather)null);

        data.immigrationRequired = xr.getAttribute(IMMIGRATION_REQUIRED_TAG, 12);

        data.newLandName = xr.getAttribute(NEW_LAND_NAME_TAG, (String)null);

        data.independentNationName = xr.getAttribute(INDEPENDENT_NATION_NAME_TAG,
                                                (String)null);

        data.attackedByPrivateers = xr.getAttribute(ATTACKED_BY_PRIVATEERS_TAG,
                                               false);

        data.entryLocation = xr.getLocationAttribute(game, ENTRY_LOCATION_TAG,
                                                true);
    }
    ...
    protected void readChildren(FreeColXMLReader xr) throws XMLStreamException {
        // Clear containers.
        data.tension.clear();
        if (data.bannedMissions != null) data.bannedMissions.clear();
        data.stance.clear();
        data.foundingFathers.clear();
        data.offeredFathers.clear();
        data.europe = null;
        data.monarch = null;
        data.history.clear();
        ...
        if (data.nationType != null) addFeatures(data.nationType);
    ...
    protected void readChild(FreeColXMLReader xr) throws XMLStreamException {
        ...
        if (BAN_MISSIONS_TAG.equals(tag)) {
            ...
            data.europe = xr.readFreeColGameObject(game, Europe.class);

        } else if (HighSeas.getXMLElementTagName().equals(tag)) {
            data.highSeas = xr.readFreeColGameObject(game, HighSeas.class);
            ...
            data.market = xr.readFreeColGameObject(game, Market.class);
            ...
            data.monarch = xr.readFreeColGameObject(game, Monarch.class);

        } else if (TradeRoute.getXMLElementTagName().equals(tag)) {
            data.tradeRoutes.add(xr.readFreeColGameObject(game, TradeRoute.class));
